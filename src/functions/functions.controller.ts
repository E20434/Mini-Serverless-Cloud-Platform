...
// AuthGuard runs first (authenticates, populates request.user), then
// ScopesGuard (authorizes, using request.user) - order matters, Nest
// runs guards in the array order given here.
...
export class FunctionsController {
  ...
  register(@CurrentUser() user: AuthenticatedPrincipal, @Body() dto: RegisterFunctionDto) {
    return this.functionsService.register(user.userId, dto);
  }

  @Get()
  @RequireScopes('functions:read')
  list(@CurrentUser() user: AuthenticatedPrincipal) {
    return this.functionsService.list(user.userId);
  }

  @Get(':name')
  @RequireScopes('functions:read')
  get(@CurrentUser() user: AuthenticatedPrincipal, @Param('name') name: string) {
  ...
  @RequireScopes('functions:write')
  remove(@CurrentUser() user: AuthenticatedPrincipal, @Param('name') name: string) {
    ...
    return this.functionsService.remove(user.userId, name);
  }

  @Post(':name/invoke')
  @RequireScopes('functions:invoke')
  async invoke(
    ...
    // this.functionsService.invoke() throws (unknown/not-yours function
    // name), it never reaches this line at all - Nest's exception filter
    // turns that into a 404 on its own, upstream of this method entirely.
    const outcome = await this.functionsService.invoke(user.userId, name, event);
    ...
    // The core API-design lesson from Phase 3: the platform DID
